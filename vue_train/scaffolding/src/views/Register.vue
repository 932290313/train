<template>
  <div>
    ...
    <p class="ziti">根据铁路部门要求，购买火车票需要实名制，请如实填写您的证件信息</p>
    <mt-button class="bottom" size="large" type="primary" @click="handle">免费注册</mt-button>
  </div>
</template>
<style>
.field>.qian{
  ...
  methods: {
    ...
    handle() {
      if (this.checkName && this.checkCard && this.checkPhone) {
        this.axios.post('/register','realname='+this.realname+'&user_card='+this.user_card+'&phone='+this.phone)
        .then(res=>{
          if(res.data.code==1){
            this.$router.push('/')
          }else{
